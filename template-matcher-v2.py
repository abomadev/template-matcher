...
class MatchTemplate:
    def __init__(self, basepath, showtype):
        ...
        threshold, base_thresholded = cv2.threshold(self.grayscale(base_img), self.threshold,255, cv2.THRESH_BINARY)
        
        # plt.imshow(base_img)
        for filename in os.listdir(directory):
        # for filename in ["spade-template-0.png","heart-template-0.png","club-template-0.png","diamond-template-0.png"]:
            if filename.endswith(".png"):
                self.find_match(base_thresholded,os.path.join(directory, filename), showtype)
            else:
                continue
        # # 45200
        ...
        # plt.title(f'Diff - {self.best_non_zero_count}'), plt.xticks([]), plt.yticks([])
    ...
    def find_match(self, base,templatepath, showtype):
        ...
        for i in range(base_h):
            ...
            difference = cv2.subtract(shifted_base,template_base)
            ...
            if non_zero_count < self.best_non_zero_count:
                self.best_template_name = template_filename
                self.best_non_zero_count = non_zero_count
                self.best_match_img = shifted_base
                ...
                if showtype == 'best':
                    # Show improvements
                    print(f'Best:{self.best_template} - {self.best_shift} - {self.best_non_zero_count} | Curr: {template_filename} - {non_zero_count} - {non_zero_count}')
                    self.fg.suptitle(f'Matching:{template_filename} BM:{non_zero_count}')
                    self.h.set_data(shifted_base)
                    draw(), pause(1e-4)
                    # self.show_match(img,template,max_loc,w,h)
            if showtype == 'all':
                # Show improvements
                self.h.set_data(difference)
                # self.fg.suptitle(f'Matching:{template_filename} BM:{non_zero_count}')
